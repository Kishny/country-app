...
class GlobalExplorer {
  ...
  async fetchCountries() {
    ...
    try {
      // Utilisation d'un proxy CORS pour éviter les problèmes
      const proxyUrl = "https://api.allorigins.win/get?url=";
      const encodedUrl = encodeURIComponent(this.config.apiEndpoint);
      const response = await fetch(`${proxyUrl}${encodedUrl}`);
      const data = await response.json();

      // Parse les données depuis le proxy
      if (!data || typeof data.contents !== "string") {
        console.error("Proxy AllOrigins invalide:", data);
        throw new Error("Proxy invalide : data.contents manquant.");
      }
      ...
      try {
        ...
        console.error("Impossible de parser data.contents:", data.contents);
        throw new Error("Le proxy a renvoyé un contenu non JSON.");
      }
      ...
      // Parse les données depuis le proxy
      this.countries = JSON.parse(data.contents);

      // Extraction des régions et sous-régions
      this.extractRegions();
      ...
      // Chargement des données de secours
