...
class GlobalExplorer {
  ...
  async fetchCountries() {
    ...
    try {
      ...
      const directUrl = `https://restcountries.com/v3.1/all?fields=${FIELDS}`;

      // ✅ AllOrigins RAW = pas besoin de JSON.parse(contents)
      const proxyRaw = `https://api.allorigins.win/raw?url=${encodeURIComponent(
        directUrl
      )}`;
